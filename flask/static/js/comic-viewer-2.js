...
comic.newGetImgPath = function(dir, no, count) {
	if (no < 1 || no > count) {
		return null;
	} else {
		//console.log(no)
		//console.log(this.param.files[no])
 		return imgPath = dir + '/' + this.param.files[no-1];
	}
};
...
comic.showImage = function() {
	...
	var rightPath = this.newGetImgPath(this.param.dir, this.param.no, this.param.count);
	var leftPath = this.newGetImgPath(this.param.dir, this.param.no + 1, this.param.count);
...
comic.pcInit = function() {
	...
	comic.param.no = (comic.param.start == 'l') ? 1 : 2;

	comic.param.files = $('#comic-files').val().split(',');
	console.log(comic.param.files)
...
comic.spInit = function(windowWidth) {
	...
	for (var i = 1; i <= comic.param.count; i++) {
		var imgPath = comic.newGetImgPath(this.param.dir, i, this.param.count);
